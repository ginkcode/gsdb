            ...
            if (editing) {
                ...
                if (editing.ssh) {
                    ...
                    sshPrivateKeyPassphrase =
                        editing.ssh.privateKeyPassphrase ?? "";
                } else {
                    sshHost = "";
                    sshPort = 22;
                    sshUsername = "";
...
<Dialog.Root bind:open>
    <Dialog.Content class="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div class="grid gap-4 py-2">
            ...
            <div class="grid gap-1.5">
                <label class="text-sm font-medium" for="conn-driver"
                    >Driver</label
                >
                <Select.Root
                    type="single"
            ...
            {#if driver !== "sqlite"}
                <div class="grid grid-cols-3 gap-2">
                    <div class="col-span-2 grid gap-1.5">
                        <label class="text-sm font-medium" for="conn-host"
                            >Host</label
                        >
                        <Input id="conn-host" bind:value={host} />
                    </div>
                    <div class="grid gap-1.5">
                        <label class="text-sm font-medium" for="conn-port"
                            >Port</label
                        >
                ...
                <div class="grid gap-1.5">
                    <label class="text-sm font-medium" for="conn-db"
                        >Database</label
                    >
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="grid gap-1.5">
                        <label class="text-sm font-medium" for="conn-user"
                            >Username</label
                        >
                        <Input id="conn-user" bind:value={username} />
                    </div>
                    <div class="grid gap-1.5">
                        <label class="text-sm font-medium" for="conn-pass"
                            >Password</label
                        >
                        <Input
                            id="conn-pass"
                    ...
                    <div class="grid gap-3 pl-6 border-l-2 border-muted ml-1">
                        ...
                        <div class="grid gap-1.5">
                            ...
                            <p class="text-xs text-muted-foreground">
                                Paste your private key or leave empty to use SSH
                                agent
                            </p>
                        </div>
                        <div class="grid gap-1.5">
                            <label
                                class="text-sm font-medium"
                                for="ssh-key-pass">Key Passphrase</label
                            >
                            <Input
                                id="ssh-key-pass"
                                type="password"
            ...
            {:else}
                <div class="grid gap-1.5">
                    <label class="text-sm font-medium" for="conn-path"
                        >File Path</label
                    >
                    <Input
                        id="conn-path"
